...
<form name="trashform" action={'restoretrash/trash/'|ezurl} method="post" >
...
{if $restored_nodes}
	<div class="restored-nodes">
		<h2>Restored nodes:</h2>
		<table class="list" cellspacing="0">
			<tr>				
				<th>{'Name'|i18n( 'design/admin/content/trash')}</th>
				<th>{'Type'|i18n( 'design/admin/content/trash')}</th>				
				<th>{'Placement'|i18n( 'design/admin/content/trash')}</th>				
			</tr>			
			{foreach $restored_nodes as $node}
				<tr>
					<td>
						<a href="{$node.url_alias|ezurl('no')}">{$node.name}</a>
					</td>
					<td>{$node.contentobject.class_name|wash}</td>
					<td>{$node.path_string}</td>
				</tr>
			{/foreach}			
		</table>
	</div>
...
<div class="button-left">
    {if $list_count}
		...
		*}
		<input class="button" type="submit" name="RestoreSubtreeButton"  value="{'Restore subtree'|i18n( 'design/admin/content/trash' )}" title="{'Restore parent node and it subtree'|i18n( 'design/admin/content/trash' )}" />
    {else}
